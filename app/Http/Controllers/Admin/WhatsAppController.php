...
class WhatsAppController extends Controller
{
    ...
    {
        $base = config('services.wa_bot.url', 'http://127.0.0.1:3000');
        ...
        try {
            $statusRes = Http::timeout(5)->get($base.'/status');
            $status = $statusRes->json();
            $statusText = is_array($status) ? ($status['status'] ?? 'Unknown') : ($status ?? 'Unknown');
            $isConnected = isset($status['status']) && in_array(strtolower($status['status']), ['connected']);
            ...
            if (!$isConnected) {
                try {
                    ...
                    // If QR is not available yet, check for waiting message
                    if (!$qr && isset($qrResp['waiting'])) {
                        $error = 'QR code wordt gegenereerd, probeer over een paar seconden opnieuw...';
                    }
                } catch (\Throwable $qrError) {
                    $error = 'Kon QR-code niet ophalen: ' . $qrError->getMessage();
                }
            }
        } catch (\Throwable $e) {
            $status = ['status' => 'Bot not reachable', 'error' => $e->getMessage()];
            $error = 'Bot niet bereikbaar: ' . $e->getMessage();
        }

        return view('admin.whatsapp.index', compact('status','qr','base','isConnected','error'));
    }
    ...
    public function qr()
    {
        ...
        try {
            $res = Http::timeout(5)->get($base.'/get-qr');
            return response()->json($res->json());
        } catch (\Throwable $e) {
            return response()->json(['message' => 'Bot not reachable', 'error' => $e->getMessage()], 500);
        }
    }

    /**
     * JSON: fetch status (for AJAX calls)
     */
    public function status()
    {
        ...
        try {
            ...
            return response()->json(['status' => 'Bot not reachable', 'error' => $e->getMessage()], 500);
        }
    }
