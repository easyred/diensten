...
<head>
    ...
    <title>Account Aanmaken - {{ $brandName }}.pro</title>
    <meta name="description" content="Maak een account aan op {{ $brandName }}.pro">
    ...
    <section class="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="text-center mb-8">
                ...
                <h1 class="text-3xl sm:text-4xl font-bold text-gray-900 mb-2" id="headerTitle">{{ $category ? ($category->name . ' Account Aanmaken') : 'Account Aanmaken' }}</h1>
                <p class="text-base sm:text-lg text-gray-600" id="headerSubtitle">Kies uw diensten en vul uw gegevens in om te beginnen</p>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-gray-100">
                <form method="POST" action="{{ route('register.store') }}" class="space-y-6" novalidate>
                    ...
                    <!-- Category Selection -->
                    <div>
                        <label for="category_select" class="block text-sm font-medium text-gray-700 mb-2">Selecteer Diensten *</label>
                        <div class="relative">
                            <select id="category_select" name="categories[]" multiple size="4" class="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition text-sm" required style="--tw-ring-color: {{ $primaryColor }};">
                    ...
                    <fieldset class="border border-gray-200 rounded-xl p-4 sm:p-6 bg-gray-50">
                        <legend class="text-xs font-bold uppercase tracking-wider text-gray-700 px-2">Jouw Gegevens</legend>
                    ...
                    <!-- CONTACT -->
                    <fieldset class="border border-gray-200 rounded-xl p-4 sm:p-6 bg-gray-50">
                        <legend class="text-xs font-bold uppercase tracking-wider text-gray-700 px-2">Contact</legend>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                            <!-- WhatsApp Number -->
                            <div>
                            ...
                            <!-- BTW Number -->
                            <div id="btw_block" style="display:none">
                                <label for="btw_number" class="block text-sm font-medium text-gray-700 mb-2">BTW-nummer <span class="text-gray-500 text-xs">(optioneel)</span></label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    ...
                                    <input id="btw_number" 
                                           name="btw_number" 
                                           value="{{ old('btw_number') }}" 
                                           placeholder="BE 0123.456.789"
                                           class="block w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition text-sm"
                                           style="--tw-ring-color: {{ $primaryColor }};">
                                </div>
                                <p class="mt-1 text-xs text-gray-500">Vereist voor service providers die klanten factureren.</p>
                            </div>
                        </div>
                    </fieldset>
            ...
            function updateUI() {
                ...
                if (selected.length === 0) {
                    ...
                    @if($preselectedCategory)
                        if (preselectedCategory) {
                            headerTitle.textContent = preselectedCategory.name + ' Account Aanmaken';
                        } else {
                    ...
                    @if($preselectedCategory)
                        if (preselectedCategory && preselectedCategory.code === code) {
                            headerTitle.textContent = preselectedCategory.name + ' Account Aanmaken';
                        } else {
                            headerTitle.textContent = config.title;
                        }
                ...
                if (company) {
                    ...
                    if (label) {
                        label.innerHTML = hasServiceProvider 
                            ? 'Bedrijfsnaam <span class="text-gray-500 text-xs">(optioneel maar aanbevolen)</span>' 
                            : 'Bedrijfsnaam <span class="text-gray-500 text-xs">(optioneel)</span>';
                    }
                }
            }
