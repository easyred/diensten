...
<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" novalidate>
                ...
                <div class="mt-2">
                    ...
                    @if (session('status') === 'verification-link-sent')
                        <div class="alert alert-success mt-2">
                            {{ __('Een nieuwe verificatielink is naar uw e-mailadres gestuurd.') }}
                        </div>
                    @endif
                </div>
            @endif
        ...
        {{-- WhatsApp number --}}
        <div class="mb-3">
            <label for="whatsapp_number" class="form-label">WhatsApp nummer</label>
        ...
        {{-- ADRES (dezelfde velden/gedrag als op uw registratiewebpagina) --}}
        <div class="mb-3">
            <label for="address" class="form-label">Straatadres</label>
            <div class="position-relative">
                <input id="address" name="address" type="text" class="form-control"
                       value="{{ old('address', $user->address) }}"
                       placeholder="Begin met het typen van straat, nummer, stad…" autocomplete="off" required>
            ...
            <div class="form-text">Begin met typen en kies een adres om de velden hieronder automatisch in te vullen.</div>
            @error('address')
                <div class="text-danger mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <input id="city" name="city" type="text" class="form-control"
                       value="{{ old('city', $user->city) }}" placeholder="Brussel">
                @error('city')
                    <div class="text-danger mt-1">{{ $message }}</div>
                @enderror
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function render(list,src){
        if(!list||!list.length){
            sugg.innerHTML='<div class="p-2 small text-muted">Geen resultaten</div>';
    ...
    input.addEventListener('input', ()=>{
        ...
        t=setTimeout(async ()=>{
            ...
            catch(e){ sugg.innerHTML=`<div class="p-2 small text-danger">Fout: ${escapeHtml(e.message)}</div>`; sugg.style.display='block'; }
        }, 300);
    });
